      ...
      <div class="container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div v-if="error" class="text-center py-12">
          <div class="bg-destructive/10 border border-destructive/20 rounded-lg p-6 max-w-md mx-auto">
            <h3 class="text-lg font-semibold text-destructive mb-2">Unable to load events</h3>
        ...
        <div 
          :ref="(el) => { if (el) infiniteScroll.sentinelRef.value = el }"
          class="mt-8 p-4 flex justify-center min-h-[100px]"
        >
          <div v-if="infiniteScroll.isFetchingNextPage" class="flex items-center gap-2 text-sm text-muted-foreground">
            <div class="h-4 w-4 border-2 border-primary border-t-transparent rounded-full animate-spin"></div>
            <span>Loading more events...</span>
          </div>
          <div v-else-if="!infiniteScroll.hasNextPage && events.length > 0" class="text-sm text-muted-foreground">
